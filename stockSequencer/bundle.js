...
function initTone (stock) {
	...
	Tone.Transport.setInterval(function(time){			
		stepNum++;
		stepNum = stepNum % stock.steps.length;
		synth.triggerAttackRelease(stock.steps[stepNum], '16n');
	}, "8n");
...
Stock.prototype.init = function() {
	...
	.done(function(data) {
		// console.log("success");
		// console.log(data);
		// console.log(data.Elements[0].DataSeries.close.values);

		var dates = data.Dates;
		dates = dates.slice(Math.max(dates.length - stepCount));

		var prices = data.Elements[0].DataSeries.close.values;
		prices = prices.slice(Math.max(prices.length - stepCount));

		self.init_Steps(prices, stepCount);
		self.init_Graph(prices, dates);
	})
	.fail(function() {
		console.log("error");
	})
...
Stock.prototype.init_Graph = function (prices, dates) {
	this.graphData = {
		labels: dates,
		series: [ prices ]
	};

	Chartist.Line('.ct-chart ct-major-sixth', this.graphData);

	// console.log(Chartist.Line('.ct-chart', this.graphData));
...
Stock.prototype.init_Steps = function(prices, numSteps) {
	...
	var min = _.min(this.prices);
	var max = _.max(this.prices);

	this.steps = [];
	this.music = new Music(referenceFreq);
	for(var note = 0; note < this.prices.length; note++){
		this.steps[note] = this.music.snapToNote( ( this.prices[note] ), min*.9, max*1.25 );
	}
}

},{"./music":2,"chartist":3,"jquery":4,"underscore":5}],2:[function(require,module,exports){
'use_strict';
